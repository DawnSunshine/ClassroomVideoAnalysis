...
void video_read_student(std::string path, bool resize_flag, cv::Size resize)
{
    ...
    while (true)
    {
        ...
        }
        {
            ...
            //////////////////////////////////////////
            student_resize_frame = resize_frame.clone();

        }
        //cv::imshow("student_video", resize_frame);
        boost::asio::deadline_timer t(io, boost::posix_time::milliseconds(40));
        t.wait();
        //if (cv::waitKey(30) == 27)
        //	break;
    }
...
void video_read_teacher(std::string path, bool resize_flag, cv::Size resize)
{
    ...
    while (true)
    {
        ...
        }
        {
            ...
            //////////////////////////////////////////
            teacher_resize_frame = resize_frame.clone();
...
int main()
{
    /*std::string video_path_1, video_path_2, data_save_path;
    std::cout << "Input Student Video Path:" << std::endl;
    std::cin >> video_path_1;
    std::cout << "Input Teacher Video Path:" << std::endl;
    std::cin >> video_path_2;*/

    boost::thread thread(classroom_analysis);
    boost::thread thread1(boost::bind(video_read_student,
                                      "/home/itc/test_video/student_panorama.avi", false, cv::Size(100, 100)));
    //boost::thread thread1(boost::bind(video_read_student,
    //    video_path_1, false, cv::Size(100, 100)));
    boost::thread thread2(boost::bind(video_read_teacher,
                                      "/home/itc/test_video/teacher_panorama.avi", true, cv::Size(480, 270)));
    //boost::thread thread2(boost::bind(video_read_teacher,
    //    video_path_2, true, cv::Size(480, 270)));

    cv::namedWindow("teacher");
    ...
    while(1)
    {
        ...
        {
            boost::recursive_mutex::scoped_lock lk(cs);
            if(teacher_resize_frame.size().area() > 0
                    && student_resize_frame.size().area() > 0)
            {

                cv::imshow("teacher",teacher_resize_frame);
                cv::imshow("student",student_resize_frame);
            }
        }
        if(cv::waitKey(40) == 27)
            break;
    ...
    ofs.open("json_data/student.json");
    ofs << student_json_root.toStyledString();
    ofs.close();
    ofs.open("json_data/teacher.json");
    ofs << teacher_json_root.toStyledString();
    ofs.close();
    ofs.open("json_data/time.json");
